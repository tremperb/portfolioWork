...
struct list* list_create() {
	struct list* linked_list = malloc(sizeof(struct link));
...
void list_free(struct list* list) {
	...
	while (temp != NULL) {
		remove = temp->next;	//store our next value
		free(temp);				//delete our current value
		temp = remove;			//reassign value to next one to be deleted
	}
...
void list_insert(struct list* list, void* val) {
	...
	struct link* node = malloc(sizeof(struct link));	//allocate memory

	node->val = val;		//set our current node to the given value
	node->next = list->head;	//set next to the head of the list
	list->head = node;			//now store the node
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
	...
	temp = list->head;
	struct link* previous;// = malloc(sizeof(struct link));
	previous = list->head;
	int count = 0;

	while (temp != NULL) {
		if (cmp(temp->val, val) == 0) { //if values are equal break
			break;
		}
		previous = temp;

		temp = temp->next;	//get next values to test
		count++;
	}
	//now we have got the matched value held in temp
	if (count == 0)						//have to add this case. if its the first there is no previous to replace it so just make it the head
		list->head = temp->next;
	else
		previous->next = temp->next;  // previous value saved so next value is matched, so just make that the value after temp
	free(temp);		//now delete that value
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
	...
	temp = list->head;
	while (temp != NULL) {
		if (cmp(temp->val, val) == 0) {	//if they match then the node is that position so return
			return position;
		}
		temp = temp->next;
		position++;
	...
	return -1;	//if it got this far there was no match so return -1
}
...
void list_reverse(struct list* list) {
	struct link* temp;// = malloc(sizeof(struct link));
	struct link* old;// = malloc(sizeof(struct link));
	struct link* next;// = malloc(sizeof(struct link));
	temp = list->head;
	old = NULL;	//need empty for now
	next = NULL;

	while (temp != NULL) {
		next = temp->next;	//store our next variable
		temp->next = old;	//make the previous our next now
		old = temp;			//make the old value our current
		temp = next;		//now make our current value the next
	}
	list->head = old;		//reinitialize our list
