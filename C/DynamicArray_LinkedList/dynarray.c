/* Dynamic Array Final - 10/14/2019
...
struct dynarray* dynarray_create() {
	struct dynarray* arr_ptr = malloc(2 * sizeof(void*)); //create first struct arr i.e. data, cap, size
	arr_ptr->data = malloc(2 * sizeof(void*));				//make array for all data held
	arr_ptr->capacity = 2;
	arr_ptr->size = 0;
...
void dynarray_free(struct dynarray* da) {
	free(da->data);	//delete the data
	free(da);			//delete the whole struct
	return;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
	if ( da->size == da->capacity-1) {		//if the array is full
		da->capacity = da->capacity * 2;		//then update the capacity

		struct dynarray* arr_tmp = malloc( da->capacity * sizeof( void*));	//temporary array to store
		arr_tmp->data = malloc( da->capacity * sizeof( void*));
		for ( int i = 0; i < da->capacity; i++) {		//store values unless it reaches null
			if( da->data[i] != NULL)
				arr_tmp->data[i] = da->data[i];
		}
		free(da->data);	//free previous data
		da->data = arr_tmp->data;
		free(arr_tmp);		//free temporary array

	}

	da->data[da->size] = val;	//set new value at that index
	da->size++;						//new value so need to increase size
	return;
}
...
void dynarray_remove(struct dynarray* da, int idx) {
	...
	for (int i = idx; i < da->size; i++) {
		da->data[i] = da->data[i + 1];		//remove previous and overwrite with new
	}
	da->data[da->size+1] = NULL;				//set last value to NULL which was holding a value
...
void dynarray_set(struct dynarray* da, int idx, void* val) {
	da->data[idx] = val; 	//set value at that index to given
