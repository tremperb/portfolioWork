...
struct dynarray* dynarray_create() {
  ...
  da->position = 0; //starting position is 0
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    new_data[i] = da->data[physical_index(da, i)];
  }
  da->position = 0; //reset position
  /*
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  da->data[physical_index(da, da->size)] = val;
  da->size++;
}
...
/*
 * Remove and return our value
 */
void* single_remove(struct dynarray* da, int idx) {
  assert(da);
  assert(idx < da->size && idx >= 0);
  void* val = dynarray_get(da, idx);
  da->size--;
  da->position++;
...
void* dynarray_get(struct dynarray* da, int idx) {
  ...
  return da->data[physical_index(da, idx)];
}
...
/*
 * get our physical index 
 */
int physical_index(struct dynarray* da, int idx) {
  return ((da->position + idx) % da->capacity);
}