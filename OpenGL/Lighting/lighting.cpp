...
Display( )
{
	...
	*/
	posx = 0.;
	posy = 0.;
	posz = -30. * Time; // This one move with the lightsaber
	glPushMatrix();
	//
	glColor3f(0.12, 0.92, 0.99); 
	glTranslatef(posx, posy, posz);
	glutSolidSphere(0.5, 50, 50);
	SetSpotLight(GL_LIGHT2, posx, posy, posz, -posx, -posy, posz, 0.12, 0.92, 0.99);	// spot light to need direction
	//
	glPopMatrix();
	...
	*/
	...
	glTranslatef(posx, posy, posz);
	glutSolidSphere(0.5, 50, 50);
	// Set up red spot light to shine downish after tarus
	SetSpotLight(GL_LIGHT3, posx, posy, posz, -posx, -posy, 2., 1., 0., 0.);
	//
	glPopMatrix();
	...
	/* We need 3 lights so lets set up light 1 2 and 3*/
	glPushMatrix();
	...
	*/
	glPushMatrix();
	glShadeModel(GL_SMOOTH);	// our smooth tarus
	SetMaterial(1., 1., 1., 50);
	glTranslatef(0., 0., -20.);
	glColor3f(1., 1., 1.);
	glutSolidTorus(6., 8., 50., 50.);
	glPopMatrix();

	/*
	** standard circle with flat shading and not shiny
	*/
	glPushMatrix();
	glShadeModel(GL_FLAT);
	SetMaterial(1., 0., 0., 1.);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Project 4 - Lighting by Brayden Tremper" );
