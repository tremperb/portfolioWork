...
const char *WINDOWTITLE = { "Project 2 -- Helicopter -- Brayden Tremper (Sample code by Joe Graphics)" };
...
Display( )
{
	...
	// If we aren't in standard view
	if (WhichTexture != STANDARDT) {
	...
	/*
	// Draw the lighsaber out front (from project 1)
	glPushMatrix();

	glTranslatef(0., 5, -20);
	glScalef(0.5, 0.5, 0.5);
	glCallList( BoxList );
	//Just have lightsaber as box list
	glPopMatrix();*/

	// No display list just calling our draw sphere
	//glPushMatrix();
	
	// Good now draw our hellicopter
	//glCallList(heliBodyList);
	/*

	glPushMatrix();
	// Top Blade First
	glTranslatef(0., 2.9, -2.); //4. Translate to position
	glRotatef(360.*Time, 0., 1., 0.);	//3. rotate spin (Animate)
	glRotatef(90., 1., 0., 0.); //2. roate to proper orientation
	glScalef(5., 5., 5.);	//1. scale blade to right size
	...
	glPushMatrix();
	// Top Blade First
	glTranslatef(0.5, 2.5, 9.); //4. Translate to position
	glRotatef((3. * 360.) * Time, 1., 0., 0.);	//3. 3x faster (Animate)
	glRotatef(90., 0., 1., 0.); //2. roate to proper orientation
	glScalef(1.5, 1.5, 1.5);	//1. scale blade to right size
	glCallList(bladesList);
	glPopMatrix();
	*/
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Project 3 - Texture Distortion by Brayden Tremper" );
...
InitGraphics( )
{
	texture = BmpToTexture("cube.bmp", &textureWidth, &textureHeight);
